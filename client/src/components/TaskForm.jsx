...
const TaskForm = () => {
    ...
    const handleSubmit = async (values, { resetForm }) => {
        try {
            ...
            const { title, description, subtasks, status } = values;
            const id = params_row?.id;
            if (!id) {
                createTask({ title, description, subtasks })
                    .then((res) => {
                ...
                updateTask({ id: id, updateTask: { title: title, description: description, subtasks: subtasks, status: status } })
                    .then((res) => {
                        const status = res?.data?.status;
                        const message = res?.data?.message;
                        if (status) {
                        ...
                        <Formik enableReinitialize initialValues={initialValues} validationSchema={validationSchema} onSubmit={handleSubmit}>
                            <Form className='w-full space-y-5 p-4 text-black'>
                                <div className='w-full space-y-2'>
                                    ...
                                    <Field className="p-2 rounded border w-full outline-[#322F64]" type="text" placeholder='Enter board name eg. taskWave' name="title" />
                                ...
                                <div className="w-full space-y-3">
                                    ...
                                    <Field as="select" rows='4' className="w-full p-2 rounded border outline-[#322F64]" name='status'>
                                ...
                                <div className='w-full grid grid-cols-1 md:grid-cols-2 gap-5'>
                                    ...
                                    <button className='w-full p-3 rounded shadow bg-[#322F64] text-white' type='submit'>{!params_row?.id ? 'Create Task' : 'Update Task'}</button>
                                </div>
                            </Form>
                        </Formik>
                    </div>
